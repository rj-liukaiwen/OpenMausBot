...
/** Keep the downstream Ruijie noVNC bridge separate from the new
 * agent-browser integration. The opaque capability is scoped to this bot and
 * expires even if the provider process exits without a cleanup callback. */
async function ruijieDesktopIntegration(
  botId: string,
  threadId: string,
  generation: string,
  joinUrl: string,
  ...
  return {
    ...
    env: {
      ...AGENTS_NODE_FLAG,
      OMB_DESKTOP_URL: connection.url,
      OMB_DESKTOP_TOKEN: capability.token,
      OMB_BOT_ID: botId,
...
function observedLocalComputer(
  cua: LocalComputerConnection,
  botId: string,
  threadId: string,
  generation: string,
  ...
  void (async () => {
    try {
      ...
      if (ruijieDesktopJoinUrl) {
        const ruijieDesktop = await ruijieDesktopIntegration(
          bot.id,
          threadId,
          dispatchClaimId,
...
const handleRequest = async (req: IncomingMessage, res: ServerResponse) => {
  ...
  try {
    ...
    // enabled switch and Claude account settings. Only this idle instance is
    // replaced; siblings keep running.
    ...
    if (method === "PATCH" && instancePatch) {
      ...
      try {
        let result = body.cli === undefined ? { ok: true, config: cfg } : withInstanceCli(cfg, instanceId, body.cli);
        if (result.ok && body.enabled !== undefined) result = withInstanceEnabled(result.config, instanceId, body.enabled);
    ...
    if ((method === "PUT" || method === "PATCH") && path === "/api/config") {
      ...
      if (shouldReloadProviders) revokeAllInternalCapabilities();
