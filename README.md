> **私有测试发行仓库**：基于 `WYunS/OpenMausBot` 的 `codex/openmaus-upgrade-0.1.71-ready`。v0.1.73 三平台构建及自动验收已通过：[Artifacts](https://github.com/rj-liukaiwen/OpenMausBot/actions/runs/34554472677) · [私有 Release 草稿](https://github.com/rj-liukaiwen/OpenMausBot/releases/tag/untagged-bbbc55a55c3d583dd24b)。使用 [私有测试一键发布指南](发布交付指南/04-私有测试一键发布.md) 和 [验证记录](docs/verification/private-release-0.1.73.md)。Windows 未签名；macOS Universal 为 ad-hoc 签名、未公证，实体机验收待执行。下方保留上游项目说明。
...
&nbsp;
<a href="https://github.com/milind-soni/OpenMausBot/releases/latest/download/RuijieBot-setup.exe">
...
- **Local first.** One small harness server on `127.0.0.1` owns every agent process. Transcripts, keys, and
  events live locally, not in a cloud. Source/CLI builds use `~/.openmausbot`; packaged RuijieBot builds use
  the isolated `~/.ruijiebot` directory on Windows and macOS.
