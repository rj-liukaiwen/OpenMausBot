...
if (!/^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)$/.test(version)) throw new Error("Version must have the form v0.1.74 or 0.1.74.");
const tag = `v${version}`;
...
if (!infoResponse.ok || (await infoResponse.json()).private) throw new Error("This evaluation configuration expects the owner-authorized public repository.");
...
writeFileSync("build/private-release.json", `${JSON.stringify({ schemaVersion: 1, version, sourceSha, upstreamSha: "d7cd142712c11f44a7a13d9e75c8d31ad9799891", repository, workflowRun: process.env.GITHUB_RUN_ID, signing: { windows: "unsigned", macos: "ad-hoc signed, not notarized" }, usage: "public test candidate; Enterprise redistribution authorized by repository owner", humanAcceptance: "skipped-by-owner", acceptancePolicy: "legacy-evaluation", dataDirectory: "~/.ruijiebot" }, null, 2)}\n`);
git("config", "user.name", "github-actions[bot]");
git("config", "user.email", "41898282+github-actions[bot]@users.noreply.github.com");
git("add", "package.json", "build/private-release.json");
if (git("diff", "--cached", "--name-only")) git("commit", "-m", `build: prepare RuijieBot evaluation ${tag}`);
...
appendFileSync(process.env.GITHUB_STEP_SUMMARY, `### RuijieBot evaluation ${tag}\n\nSource: \`${sourceSha}\`\n\nBuild commit shared by all platforms: \`${sha}\`\n\nWindows unsigned; macOS ad-hoc signed, not notarized. Human acceptance skipped by owner; automated integrity and native smoke checks remain required.\n`);
