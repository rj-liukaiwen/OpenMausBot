...
| Windows x64 | [Download installer](https://github.com/milind-soni/OpenMausBot/releases/latest/download/RuijieBot-setup.exe) | Per-user installer; Windows signing is not yet available. |
...
Source and CLI builds store application data under `~/.openmausbot`. Packaged RuijieBot builds use the isolated `~/.ruijiebot` directory on Windows and macOS. Electron window and browser data use the normal per-platform application data directory.
